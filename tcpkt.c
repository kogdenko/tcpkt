                ...
                enum proto_node_type type, char *s, char *eb)
{
	...
	if ((s = parse_node_name(name_buf, s, eb)) == NULL)
		return NULL;
...
compar_ARP_REQUEST(struct pkt *p, struct pkt *r)
{
	...
	if (data->sip != play.raddr.ipv4) {
		return -1;
	}

	return memcmp(&data->tha, &dev.hwaddr, sizeof(data->tha));
}
...
parse_ARP_REPLY(struct pkt *p, struct proto_node *node, char *s, char *eb)
{
	...
	data->tip = play.raddr.ipv4;
	data->tha = play.gateway;
	data->sip = play.laddr.ipv4;
	data->sha = dev.hwaddr;
...
compar_ARP_REPLY(struct pkt *p, struct pkt *r)
{
	...
	return memcmp(&data->sha, &play.gateway, sizeof(data->sha));
}
...
parse_pkt(struct pkt *p, char *s, char *eb)
{
	...
	if (p->req == PKT_MANDATORY) {
		...
		if (parse_interval(scope, &p->delay)) {
	...
	return parse_root(p, s, eb) == NULL ? -1 : 0;
}
...
send_pkt(struct pkt *p)
{
	...
	p->pcb.eth_saddr = dev.hwaddr;
	p->pcb.eth_daddr = play.gateway;
	p->dir = PKT_OUTGOING;
	...
	if (p->pcb.eth_type != ETH_TYPE_ARP) {
		...
		addr.sin_addr.s_addr = play.raddr.ipv4;
		addr.sin_port = play.rport;
	}
...
match_laddr(const ipaddr_t *laddr)
{
	...
	inet_ntop(af, laddr->data, dst, sizeof(dst));
	ldbg(60, "missmatch laddr: %s\n", dst);

	return -1;	
}

static int
match_raddr(const ipaddr_t *raddr)
{
	int af;
	char dst[INET6_ADDRSTRLEN];
	...
	if (!ipaddr_cmp(af, raddr, &play.raddr)) {
		return 0;
	}

	inet_ntop(af, raddr->data, dst, sizeof(dst));
	ldbg(60, "missmatch raddr: %s\n", dst);

	return -1;	
}

static int
match_ports(be16_t lport, be16_t rport)
{
	if (play.lport != 0 && play.lport != lport) {
		ldbg(60, "missmatch lport: %hu != %hu\n",
			BE16_TO_CPU(lport), BE16_TO_CPU(play.lport));
		return -1;
	}

	if (play.rport != 0 && play.rport != rport) {
		ldbg(60, "missmatch rport: %hu != %hu\n",
			BE16_TO_CPU(rport), BE16_TO_CPU(play.rport));
		return -1;
	}
...
recv_arp(struct pkt *r)
{
	...
	if (r->pcb.arp.op == ARP_OP_REQUEST) {
		...
		data = &r->pcb.arp.ipv4;
		if (data->tip == play.laddr.ipv4 &&
		    data->sip == play.next_hop) {
			play.gateway = data->sha;
		}
	}
}
...
arp_resolved()
{
	return memcmp(&play.gateway, &eth_bcast, sizeof(eth_bcast));
}

static int
arp_resolve(unsigned int probe_to)
{
	int i;
	unsigned int to;
	struct pkt r, p;
...
validate_script(struct pkt **pkts, int nr_pkts, const char *filename)
{
	...
	for (i = 0; i < nr_pkts; ++i) {
		...
		if (fill_pkt(&dev, pkt_buf, &p->pcb, NULL) == -1) {
...
{
	printf(
	"Usage: tcpkt [options] {-i interface} {remote-ip} {script}\n"
	"\n"
	"\tOptions:\n"
	"\t-h                 print this help\n"
	"\t-v                 be verbose\n"
...
{
	int opt, nr_pkts;
	unsigned int lport, rport, failed, wait_ms;
	be32_t raddr;
