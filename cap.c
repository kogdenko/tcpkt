...
#ifdef WITH_NETMAP
static void
cap_open(const char *ifname)
{
	struct nmreq nmr;
	char ifname_nm[IFNAMSIZ + 16];

	memset(&nmr, 0, sizeof(nmr));
	nmr.nr_rx_rings = 1;
	nmr.nr_tx_rings = 1;

	snprintf(ifname_nm, sizeof(ifname_nm), "netmap:%s", ifname);
	nmd_main = nm_open(ifname_nm, &nmr, 0, NULL);
	if (nmd_main == NULL) {
		die(errno, "nm_open('%s') failed", ifname_nm);
	}
	...
	snprintf(ifname_nm, sizeof(ifname_nm), "netmap:%s^", ifname);
	nmd_host = nm_open(ifname_nm, &nmr, 0, NULL);
	if (nmd_host == NULL) {
		die(errno, "nm_open('%s') failed", ifname_nm);
	}
}
...
static int
cap_nm_first_slot(struct nm_desc *nmd, const uint8_t **data)
{
	int i;
...
static void
cap_nm_fwd(struct nm_desc *dst, struct nm_desc *src)
{
	int i;
...
cap_recv(const uint8_t **data)
{
	...
	while (1) {
		...
		if (rc == -1) {
			...
			if (pfds[1].revents) {
				cap_nm_fwd(nmd_main, nmd_host);
			}

			if (pfds[0].revents) {
				if ((len = cap_nm_first_slot(nmd_main, data)) != 0)
					return len;
			}
		}
	}
...
static void
cap_nm_xmit(struct nm_desc *nmd, const void *data, size_t count)
{
	struct netmap_ring *txr;
	struct netmap_slot *slot;
...
void
cap_send(struct if_dev *dev, const void *data, size_t count)
{
	struct nm_desc *nmd;
	...
	cap_nm_xmit(nmd, data, count);
}

void
cap_put(const void *data, size_t count)
{
	cap_nm_xmit(nmd_host, data, count);
}
#else
static void
cap_open(const char *ifname)
{
	...
	pcap = pcap_open_live(ifname, SNAPLEN, 1, 1, eb);

//	printf("-- datalink %d\n", pcap_datalink(pcap));

	if (pcap == NULL) {
		die(0, "pcap_open_live('%s') failed (%s)", ifname, eb);
	}
}
...
cap_recv(const uint8_t **data)
{
	...
	while (1) {
		rc = pcap_next_ex(pcap, &pkt_hdr, &pkt_data);
		if (rc == 0) {
			return 0;
		}

		if (rc == -1) {
			die(0, "pcap_next_ex() failed (%s)\n", pcap_geterr(pcap));
		}

		assert(rc == 1);
		assert(pkt_hdr->caplen <= pkt_hdr->len);

		if (pkt_hdr->len > pkt_hdr->caplen) {
			ldbg(9, "packet too large: len=%u\n", pkt_hdr->len);
		} else {
			*data = pkt_data;
			return pkt_hdr->len;
		}
...
cap_send(struct if_dev *dev, const void *data, size_t count)
{
	...
	rc = pcap_inject(pcap, data, count);

        if (rc == -1) {
		die(0, "pcap_inject() failed (%s)", pcap_geterr(pcap));
	}
}
...
#ifdef __linux__
static void
get_dev_hwaddr(struct if_dev *dev)
{
	int i, fd;
	struct ifreq ifr;
	...
	if (ioctl(fd, SIOCGIFHWADDR, &ifr) == -1) {
		die(errno, "ioctl('%s', SIOCGIFHWADDR) failed", dev->ifname);
	}

	for (i = 0; i < 6; ++i) {
		dev->hwaddr.bytes[i] = ifr.ifr_addr.sa_data[i];
	}
...
get_dev_hwaddr(struct if_dev *dev)
{
	...
	if (getifaddrs(&ifa))
		die(errno, "getifaddrs() failed");

	ptr = NULL;

	for (cur = ifa; cur != NULL; cur = cur->ifa_next) {
		if (cur->ifa_addr->sa_family == AF_LINK &&
			!strcmp(dev->ifname, cur->ifa_name)) {

			ptr = (uint8_t *)LLADDR((struct sockaddr_dl *)cur->ifa_addr);
		}
	}

	if (ptr == NULL)
		die(0, "getifaddrs() doesn't return dev '%s'", dev->ifname);

	for (i = 0; i < 6; ++i) {
		dev->hwaddr.bytes[i] = ptr[i];
	}
...
dev_init(struct if_dev *dev, const char *ifname)
{
	...
	dev->ifindex = if_nametoindex(ifname);
	if (dev->ifindex == 0 && errno) {
		die(errno, "if_nametoindex('%s') failed", ifname);
	}
...
dev_recv(struct if_dev *dev, const uint8_t **data, unsigned int *to)
{
	...
	while (1) {
		...
		if (len < sizeof(*eth_h)) {
			ldbg(9, "packet too small: len=%u\n", len);
			continue;
		}

		eth_h = (struct eth_hdr *)*data;

		is_bcast = !memcmp(&eth_h->daddr, &eth_bcast, sizeof(eth_bcast));

		if (is_bcast == 0 && dev->is_loopback == 0 &&
			memcmp(&eth_h->daddr, &dev->hwaddr, sizeof(dev->hwaddr))) {

			ldbg(9, "skip packet: daddr=%s, hwaddr=%s",
				eth_ntoa(&eth_h->daddr, daddr_buf),
				eth_ntoa(&dev->hwaddr, hwaddr_buf));
			continue;
		}
...
#ifdef __linux__
static int
fill_l2_hdr(struct if_dev *dev, void *buf, struct proto_cb *pcb)
{
	return fill_eth_hdr(dev, buf, pcb);
}

static int
get_l2_hdr(struct if_dev *dev, struct proto_cb *pcb, const uint8_t *buf)
{
	return get_eth_hdr(dev, pcb, buf);
}
#else
static int
fill_l2_hdr(struct if_dev *dev, void *buf, struct proto_cb *pcb)
...
{
	if (dev->is_loopback) {
		pcb->eth_saddr = eth_zero;
		pcb->eth_daddr = eth_zero;

		switch (*(uint32_t *)buf) {
		case AF_INET:
			pcb->eth_type = ETH_TYPE_IPV4;
			break;

		case AF_INET6:
			pcb->eth_type = ETH_TYPE_IPV6;
			break;
...
fill_pkt(struct if_dev *dev, void *buf, struct proto_cb *pcb, const uint8_t *data)
{
	...
	l2_len = fill_l2_hdr(dev, buf, pcb);
...
tcp_input(struct if_dev *dev, struct proto_cb *pcb, const uint8_t *buf, size_t n)
{
	...
	pcb->l2_len = get_l2_hdr(dev, pcb, buf);

	if (rem < pcb->l2_len) {
		ldbg(10, "no space for l2 hdr\n");
		return NULL;
	}
	rem -= pcb->l2_len;
	...
	case ETH_TYPE_ARP:
		...
		if (arp_h->hrd != ARP_HRD_ETH_BE) {
			ldbg(10, "bad arp hrd %hu", BE16_TO_CPU(arp_h->hrd));
			return NULL;
		}

		if (arp_h->pro != ETH_TYPE_IPV4_BE) {
			ldbg(10, "bad arp pro %hu", BE16_TO_CPU(arp_h->pro));
			return NULL;
		}

		if (arp_h->hlen != ETH_ADDR_LEN) {
			ldbg(10, "bad arp hlen %hhu", arp_h->hlen);
			return NULL;
		}

		if (arp_h->plen != 4) {
			ldbg(10, "bad arp plen %hhu", arp_h->plen);
			return NULL;
		}
	...
	default:
		ldbg(60, "bad ether type: 0x%hx\n", pcb->eth_type);
		return NULL;
	}

	if (rem < sizeof(*ipv4_h)) {
		ldbg(10, "no space for ipv4 hdr\n");
		return NULL;
	}
	...
	if (rem < ipv4_h_len) {
		ldbg(10, "no space for ipv4 hdr opts\n");
		return NULL;
	}
	...
	case IPPROTO_TCP:
		...
		if (total_len < ipv4_h_len + tcp_h_len) {
			ldbg(10, "too small ipv4 total len\n");
			return NULL;
		}
		...
		if (rem < len) {
			ldbg(10, "no space for ipv4 total len\n");
			return NULL;
		}

		if (tcp_opt_in(&pcb->tcp.opt, tcp_h, tcp_h_len)) {
			ldbg(10, "bad tcp opts\n");
			return NULL;
		}
